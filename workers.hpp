 ...
 */
inline void dispatchCallback(const struct pfring_pkthdr *phdr, const u_char *pdata , const u_char *user_bytes){
...
public:
     ...
     */
    inline firstStage(int nw, char* device, uint promisc, int cnt, int h, uint id, uint core):id(id),core(core),end(false){
#ifdef COMPUTE_STATS
        ...
        lhsize=h/nWorkers;
        offline=false;
        handle[id]=pfring_open(device, promisc, 200, 0);
        assert(handle[id]);
        int datalinkType=1;
...
public:
     ...
     * \param core The id of the core on which this thread should be mapped.
      */
    inline genericStage(uint id, uint hSize, uint maxActiveFlows, uint idle, uint lifeTime, int flowsPerTaskCheck, uint core):
...
public:
     ...
     * \param core The id of the core on which this thread should be mapped.
     */
    inline lastStage(FILE* out,uint queueTimeout,const char* collector, uint port, uint minFlowSize, uint32_t systemStartTime, uint core):
...
private:
   static void * execFun(void * t) {((CThread *)t)->execute(); return NULL;}
...
#endif
public:
    readerThread(ff::FFBUFFER* outbuffer, firstStage *reader):outbuffer(outbuffer),reader(reader)
#ifdef COMPUTE_STATS
    ,pushlost(0)
#endif
    {;}
